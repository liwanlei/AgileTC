...
class CaseModal extends React.Component {
  ...
  handleAiGenerate = () => {
    const { aiTask, aiDocUrl, aiFile } = this.state;
    if (!aiTask && !aiDocUrl && !aiFile) {
      message.warning('请至少填写一种输入方式：需求描述 / 文档链接 / 上传文档');
      return;
    }

    this.props.form.validateFields(['case', 'bizId'], (err, values) => {
      if (err) return;
      ...
      request(`${this.props.doneApiPrefix}/case/create`, {
        method: 'POST',
        body: params,
      })
        .then(res => {
          if (res.code !== 200) {
            message.error(res.msg || '创建用例失败');
          ...
          fetch(`${this.props.doneApiPrefix}/api/case/aiRun`, {
            ...
            .then(data => {
              ...
              message.success(`AI 任务已提交，正在生成测试用例... (taskId: ${taskId})`);
              this.setState({ aiLoading: false });
              this.props.onClose(false);

              if (this.props.onAiSubmitted) {
                this.props.onAiSubmitted(caseId);
              }
            ...
            .catch(err => {
              ...
              message.error('AI 服务调用失败，请检查服务是否正常运行');
              this.setState({ aiLoading: false });
            });
        })
        .catch(err => {
  ...
  render() {
    ...
    return (
      <Modal
        ...
        onOk={this.state.createMode === 'ai' ? this.handleAiGenerate : this.handleOk}
        maskClosable={false}
        wrapClassName="oe-caseModal-style-wrap"
        title={title}
        okText={this.state.createMode === 'ai' ? (this.state.aiLoading ? '生成中...' : 'AI 生成') : '确认'}
        cancelText="取消"
        width="600px"
        confirmLoading={this.state.aiLoading}
      >
        {operate == 'add' && (
          <Form.Item {...formItemLayout} label="创建方式：">
            {getFieldDecorator('createMode', {
              initialValue: 'manual',
            })(
              <Radio.Group
                onChange={e => {
                  this.setState({ createMode: e.target.value });
                }}
                value={this.state.createMode}
              >
                <Radio.Button value="manual">手动创建</Radio.Button>
                <Radio.Button value="ai">AI 智能生成</Radio.Button>
              </Radio.Group>,
            )}
          </Form.Item>
        )}
        ...
        {this.state.createMode === 'ai' && operate == 'add' && (
          <div className="ai-generate-mode">
            ...
            <Form.Item {...formItemLayout} label="需求描述：">
              {getFieldDecorator('aiTask', {})(
                <TextArea
                  autoSize={{ minRows: 4 }}
                  placeholder="请输入需求描述，AI 将根据此内容生成测试用例"
                  maxLength="5000"
                  onChange={e => this.setState({ aiTask: e.target.value })}
                  value={this.state.aiTask}
                />,
              )}
            </Form.Item>

            <Form.Item {...formItemLayout} label="文档链接：">
              {getFieldDecorator('aiDocUrl', {})(
                <Input
                  placeholder="支持飞书/TAPD/语雀/石墨/Confluence 链接"
                  onChange={e => this.setState({ aiDocUrl: e.target.value })}
                  value={this.state.aiDocUrl}
                />,
              )}
            </Form.Item>
            ...
            <Row style={{ marginBottom: '20px' }}>
              ...
              <Col span={16}>
                <Dragger
                  accept=".md,.txt,.pdf"
                  onRemove={() => this.setState({ aiFile: null })}
                  beforeUpload={file => {
                    const isValid = /\.(md|txt|pdf)$/i.test(file.name);
                    if (!isValid) {
                      message.error('仅支持 .md / .txt / .pdf 格式');
                      return false;
                    }
                    const isLt50M = file.size / 1024 / 1024 <= 50;
                    if (!isLt50M) {
                      message.error('文档大小不能超过 50M');
                      return false;
                    }
                    this.setState({ aiFile: file });
                    return false;
                  }}
                  fileList={this.state.aiFile ? [this.state.aiFile] : []}
                >
                  {this.state.aiFile === null ? (
                    <Icon type="plus-circle" style={{ color: '#447CE6', fontSize: '24px' }} />
                  ) : (
                    <Icon type="file" style={{ color: '#447CE6', fontSize: '24px' }} />
                  )}
                </Dragger>
